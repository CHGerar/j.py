...
imagenes_rutas = [
    os.path.join(carpeta_base, "imagenes", "animal.bmp"),       # Pregunta 1
    os.path.join(carpeta_base, "imagenes", "pasillo.bmp"),      # Pregunta 2
    os.path.join(carpeta_base, "imagenes", "gemas.bmp"),        # Pregunta 3
    os.path.join(carpeta_base, "imagenes", "murcielago.bmp"),   # Pregunta 4
    os.path.join(carpeta_base, "imagenes", "charco.bmp")        # Pregunta 5
]
...
preguntas = [
    ...
    "Encuentras un pasaje oscuro y escuchas ruidos extraños.\n1- Avanzas con cuidado.\n2- Decides regresar por otro camino.",
...
respuestas_opcion1 = [
    ...
    "Te agachas y el murciélago pasa de largo.",
...
def dibujar_dialogo(texto):
    caja_h = 160
    caja = pygame.Surface((ANCHO - 40, caja_h), pygame.SRCALPHA)
    caja.fill((10, 10, 10, 200))
    pantalla.blit(caja, (20, ALTO - caja_h - 20))
    lineas = texto.split('\n')
    y_offset = 0
    for linea in lineas:
...
while True:
    ...
    if estado_juego == "instrucciones":
        ...
        if indice_pregunta == 0:
            ...
            dibujar_dialogo("¡Has llegado al final de la cueva!\nPresiona ESPACIO para ver tu destino.")

    elif estado_juego == "decision":
        pantalla.blit(imagenes_preguntas[indice_pregunta - 1], (0, 0))
        pantalla.blit(personaje_img, (personaje_x, personaje_y))
        if indice_pregunta - 1 < len(preguntas):
            dibujar_dialogo(preguntas[indice_pregunta - 1])
